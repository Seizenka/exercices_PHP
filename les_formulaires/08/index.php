<?php
    echo "<h1>PHP - Les formulaires - Exercice 8.</h1>";
    ...
    if(is_uploaded_file($_FILES['file']['name'])){
        ...
        $PDF = file_get_contents($_FILES['file']['name']);
        $PDF = substr($PDF, 0,4);//substr = retourne un segment de chaîne
        if($PDF = '%PDF'){
            $check = true;
            echo "Check : this file is a PDF";
        }
        else{
            $check = false;
            echo "Check : This file isn't a PDF";
        }
    }
...
<head>
    ...
    <title>PHP - Les formulaires - Exercice 8</title>
